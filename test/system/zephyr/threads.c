...
int metal_test_run(int threads, metal_thread_t child, void *arg)
{
	...
	metal_unused(child);
	metal_unused(arg);
	metal_err("metal_test_run is currently not implemented for Zephyr\n");
	return 1;
}
			   ...
			   void *arg, void *tids, int *threads_out)
{
	...
	metal_err("metal_test_run_noblock is currently not implemented for Zephyr\n");
	return 1;
}
...
void metal_test_finish_threads(int threads, void *tids)
{
	metal_unused(threads);
	metal_unused(tids);
	metal_err("metal_test_finish_threads is currently not implemented for Zephyr\n");
	return 1;
}
